package com.whx.jetpacktest.widget.coord.appbar;
...
public class MyAppbarLayout extends LinearLayout implements CoordinatorLayout.AttachedBehavior {
    ...
    /**
     * Interface definition for a callback to be invoked when an {@link MyAppbarLayout}'s vertical
     * offset changes.
     */
    // TODO(b/76413401): remove this base interface after the widget migration
    public interface BaseOnOffsetChangedListener<T extends MyAppbarLayout> {

        /**
         * Called when the {@link MyAppbarLayout}'s layout offset has been changed. This allows child
         * views to implement custom behavior based on the offset (for instance pinning a view at a
         * certain y value).
         *
         * @param appBarLayout the {@link MyAppbarLayout} which offset has changed
         * @param verticalOffset the vertical offset for the parent {@link MyAppbarLayout}, in px
         */
    ...
    public interface OnOffsetChangedListener extends MyAppbarLayout.BaseOnOffsetChangedListener<MyAppbarLayout> {
        void onOffsetChanged(MyAppbarLayout appBarLayout, int verticalOffset);
    }
    ...
    private List<MyAppbarLayout.BaseOnOffsetChangedListener> listeners;
    ...
    private final List<MyAppbarLayout.LiftOnScrollListener> liftOnScrollListeners = new ArrayList<>();
    ...
    /**
     * Add a listener that will be called when the offset of this {@link MyAppbarLayout} changes.
     *
     * @param listener The listener that will be called when the offset changes.]
     * @see #removeOnOffsetChangedListener(MyAppbarLayout.OnOffsetChangedListener)
     */
    @SuppressWarnings("FunctionalInterfaceClash")
    public void addOnOffsetChangedListener(@Nullable MyAppbarLayout.BaseOnOffsetChangedListener listener) {
        if (listeners == null) {
    ...
    @SuppressWarnings("FunctionalInterfaceClash")
    public void addOnOffsetChangedListener(MyAppbarLayout.OnOffsetChangedListener listener) {
        addOnOffsetChangedListener((MyAppbarLayout.BaseOnOffsetChangedListener) listener);
    }

    /**
     * Remove the previously added {@link MyAppbarLayout.OnOffsetChangedListener}.
     *
    ...
    @SuppressWarnings("FunctionalInterfaceClash")
    public void removeOnOffsetChangedListener(@Nullable MyAppbarLayout.BaseOnOffsetChangedListener listener) {
    ...
    @SuppressWarnings("FunctionalInterfaceClash")
    public void removeOnOffsetChangedListener(MyAppbarLayout.OnOffsetChangedListener listener) {
        removeOnOffsetChangedListener((MyAppbarLayout.BaseOnOffsetChangedListener) listener);
    }

    /**
     * Add a {@link MyAppbarLayout.LiftOnScrollListener} that will be called when the lift on scroll elevation and
     * background color of this {@link MyAppbarLayout} change.
     */
    public void addLiftOnScrollListener(@NonNull MyAppbarLayout.LiftOnScrollListener liftOnScrollListener) {
        liftOnScrollListeners.add(liftOnScrollListener);
    }

    /** Remove a previously added {@link MyAppbarLayout.LiftOnScrollListener}. */
    public boolean removeLiftOnScrollListener(@NonNull MyAppbarLayout.LiftOnScrollListener liftOnScrollListener) {
        return liftOnScrollListeners.remove(liftOnScrollListener);
    }

    /** Remove all previously added {@link MyAppbarLayout.LiftOnScrollListener}s. */
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, z = getChildCount(); i < z; i++) {
            ...
            final MyAppbarLayout.LayoutParams childLp = (MyAppbarLayout.LayoutParams) child.getLayoutParams();
    ...
    private boolean hasCollapsibleChild() {
        for (int i = 0, z = getChildCount(); i < z; i++) {
            if (((MyAppbarLayout.LayoutParams) getChildAt(i).getLayoutParams()).isCollapsible()) {
                return true;
            }
        }
        return false;
    ...
    @Override
    @NonNull
    public CoordinatorLayout.Behavior<MyAppbarLayout> getBehavior() {
        return new MyAppbarLayout.Behavior();
    }
    ...
    /**
     * Sets whether this {@link MyAppbarLayout} is expanded or not, animating if it has already been
     * laid out.
     *
     * <p>As with {@link MyAppbarLayout}'s scrolling, this method relies on this layout being a direct
    ...
    /**
     * Sets whether this {@link MyAppbarLayout} is expanded or not.
     *
     * <p>As with {@link MyAppbarLayout}'s scrolling, this method relies on this layout being a direct
     * child of a {@link CoordinatorLayout}.
     *
    ...
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof MyAppbarLayout.LayoutParams;
    }

    @Override
    protected MyAppbarLayout.LayoutParams generateDefaultLayoutParams() {
        return new MyAppbarLayout.LayoutParams(MyAppbarLayout.LayoutParams.MATCH_PARENT, MyAppbarLayout.LayoutParams.WRAP_CONTENT);
    }

    @Override
    public MyAppbarLayout.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new MyAppbarLayout.LayoutParams(getContext(), attrs);
    }

    @Override
    protected MyAppbarLayout.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        if (Build.VERSION.SDK_INT >= 19 && p instanceof LinearLayout.LayoutParams) {
            return new MyAppbarLayout.LayoutParams((LinearLayout.LayoutParams) p);
        } else if (p instanceof MarginLayoutParams) {
            return new MyAppbarLayout.LayoutParams((MarginLayoutParams) p);
        }
        return new MyAppbarLayout.LayoutParams(p);
    }
    ...
    public final int getTotalScrollRange() {
        ...
        for (int i = 0, z = getChildCount(); i < z; i++) {
            final View child = getChildAt(i);
            final MyAppbarLayout.LayoutParams lp = (MyAppbarLayout.LayoutParams) child.getLayoutParams();
            final int childHeight = child.getMeasuredHeight();
            final int flags = lp.scrollFlags;

            if ((flags & MyAppbarLayout.LayoutParams.SCROLL_FLAG_SCROLL) != 0) {
                ...
                if ((flags & MyAppbarLayout.LayoutParams.SCROLL_FLAG_EXIT_UNTIL_COLLAPSED) != 0) {
    ...
    int getDownNestedPreScrollRange() {
        ...
        for (int i = getChildCount() - 1; i >= 0; i--) {
            ...
            if ((flags & MyAppbarLayout.LayoutParams.FLAG_QUICK_RETURN) == MyAppbarLayout.LayoutParams.FLAG_QUICK_RETURN) {
                // First take the margin into account
                int childRange = lp.topMargin + lp.bottomMargin;
                // The view has the quick return flag combination...
                if ((flags & MyAppbarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS_COLLAPSED) != 0) {
                    // If they're set to enter collapsed, use the minimum height
                    childRange += ViewCompat.getMinimumHeight(child);
                } else if ((flags & MyAppbarLayout.LayoutParams.SCROLL_FLAG_EXIT_UNTIL_COLLAPSED) != 0) {
    ...
    void onOffsetChanged(int offset) {
        ...
        if (listeners != null) {
            for (int i = 0, z = listeners.size(); i < z; i++) {
                final MyAppbarLayout.BaseOnOffsetChangedListener listener = listeners.get(i);
                if (listener != null) {
    ...
    /**
     * Sets whether the {@link MyAppbarLayout} is liftable or not.
     *
    ...
    /**
     * Sets whether the {@link MyAppbarLayout} lifted state corresponding to {@link
     * #setLiftable(boolean)} and {@link #setLifted(boolean)} will be overridden manually.
     *
     * <p>If true, this means that the {@link MyAppbarLayout} will not manage its own lifted state and
     * it should instead be manually updated via {@link #setLifted(boolean)}. If false, the {@link
     * MyAppbarLayout} will manage its lifted state based on the scrolling sibling view.
     *
    ...
    /**
     * Sets whether the {@link MyAppbarLayout} is in a lifted state or not.
     *
    ...
    /** Returns whether the {@link MyAppbarLayout} is in a lifted state or not. */
        ...
        elevationOverlayAnimator.addUpdateListener(
                new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(@NonNull ValueAnimator valueAnimator) {
                        ...
                        for (MyAppbarLayout.LiftOnScrollListener liftOnScrollListener : liftOnScrollListeners) {
    ...
    /**
     * Sets whether the {@link MyAppbarLayout} lifts on scroll or not.
     *
     * <p>If set to true, the {@link MyAppbarLayout} will animate to the lifted, or elevated, state when
    ...
    /** Returns whether the {@link MyAppbarLayout} lifts on scroll or not. */
    public boolean isLiftOnScroll() {
        return liftOnScroll;
    }

    /**
     * Sets the id of the view that the {@link MyAppbarLayout} should use to determine whether it should
     * be lifted.
     */
    ...
    /**
     * Returns the id of the view that the {@link MyAppbarLayout} should use to determine whether it
     * should be lifted.
     */
    @IdRes
    ...
    /** A {@link ViewGroup.LayoutParams} implementation for {@link MyAppbarLayout}. */
    public static class LayoutParams extends LinearLayout.LayoutParams {
        ...
        private MyAppbarLayout.ChildScrollEffect scrollEffect;
        ...
        @RequiresApi(19)
        public LayoutParams(@NonNull MyAppbarLayout.LayoutParams source) {
            // The copy constructor called here only exists on API 19+.
            super(source);
         ...
         */
        public void setScrollFlags(@MyAppbarLayout.LayoutParams.ScrollFlags int flags) {
            scrollFlags = flags;
        }
         ...
         * @attr ref com.google.android.material.R.styleable#AppBarLayout_Layout_layout_scrollFlags
         */
        @MyAppbarLayout.LayoutParams.ScrollFlags
        public int getScrollFlags() {
            return scrollFlags;
        }

        @Nullable
        private MyAppbarLayout.ChildScrollEffect createScrollEffectFromInt(int scrollEffectInt) {
            switch (scrollEffectInt) {
                case SCROLL_EFFECT_COMPRESS:
                    return new MyAppbarLayout.CompressChildScrollEffect();
                default:
                    return null;
            }
        }
         ...
         */
        @Nullable
        public MyAppbarLayout.ChildScrollEffect getScrollEffect() {
            return scrollEffect;
        }
         ...
         */
        public void setScrollEffect(@Nullable MyAppbarLayout.ChildScrollEffect scrollEffect) {
            this.scrollEffect = scrollEffect;
        }

        /**
         * Set the interpolator to when scrolling the view associated with this {@link MyAppbarLayout.LayoutParams}.
         *
         ...
         * {@link MyAppbarLayout.LayoutParams}. Null indicates 'normal' 1-to-1 scrolling.
         *
         * @attr ref
         *     com.google.android.material.R.styleable#AppBarLayout_Layout_layout_scrollInterpolator
    ...
    /**
     * The default {@link MyAppbarLayout.Behavior} for {@link MyAppbarLayout}. Implements the necessary nested scroll
     * handling with offsetting.
     */
    // TODO(b/76413401): remove the base class and generic type after the widget migration is done
    public static class Behavior extends MyAppbarLayout.BaseBehavior<MyAppbarLayout> {

        /** Callback to allow control over any {@link MyAppbarLayout} dragging. */
        public abstract static class DragCallback extends MyAppbarLayout.BaseBehavior.BaseDragCallback<MyAppbarLayout> {}
    ...
    protected static class BaseBehavior<T extends MyAppbarLayout> extends HeaderBehavior<T> {
        private static final int MAX_OFFSET_ANIMATION_DURATION = 600; // ms

        /** Callback to allow control over any {@link MyAppbarLayout} dragging. */
        // TODO(b/76413401): remove this base class and generic type after the widget migration
        public abstract static class BaseDragCallback<T extends MyAppbarLayout> {
            /**
             * Allows control over whether the given {@link MyAppbarLayout} can be dragged or not.
             *
        ...
        private MyAppbarLayout.BaseBehavior.SavedState savedState;

        @Nullable private WeakReference<View> lastNestedScrollingChildRef;
        private MyAppbarLayout.BaseBehavior.BaseDragCallback onDragCallback;
        ...
        /**
         * Set a callback to control any {@link MyAppbarLayout} dragging.
         *
         * @param callback the callback to use, or {@code null} to use the default behavior.
         */
        public void setDragCallback(@Nullable MyAppbarLayout.BaseBehavior.BaseDragCallback callback) {
        ...
        private int getChildIndexOnOffset(@NonNull T abl, final int offset) {
            for (int i = 0, count = abl.getChildCount(); i < count; i++) {
                ...
                final MyAppbarLayout.LayoutParams lp = (MyAppbarLayout.LayoutParams) child.getLayoutParams();
                if (checkFlag(lp.getScrollFlags(), MyAppbarLayout.LayoutParams.SCROLL_FLAG_SNAP_MARGINS)) {
        ...
        private void snapToChildIfNeeded(CoordinatorLayout coordinatorLayout, @NonNull T abl) {
            ...
            if (offsetChildIndex >= 0) {
                ...
                final MyAppbarLayout.LayoutParams lp = (MyAppbarLayout.LayoutParams) offsetChild.getLayoutParams();
                final int flags = lp.getScrollFlags();

                if ((flags & MyAppbarLayout.LayoutParams.FLAG_SNAP) == MyAppbarLayout.LayoutParams.FLAG_SNAP) {
                    ...
                    if (checkFlag(flags, MyAppbarLayout.LayoutParams.SCROLL_FLAG_EXIT_UNTIL_COLLAPSED)) {
                        // If the view is set only exit until it is collapsed, we'll abide by that
                        snapBottom += ViewCompat.getMinimumHeight(offsetChild);
                    } else if (checkFlag(
                            flags, MyAppbarLayout.LayoutParams.FLAG_QUICK_RETURN | MyAppbarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS)) {
                    ...
                    if (checkFlag(flags, MyAppbarLayout.LayoutParams.SCROLL_FLAG_SNAP_MARGINS)) {
        ...
        private void updateAccessibilityActions(
            ...
            if (!(lp.getBehavior() instanceof MyAppbarLayout.ScrollingViewBehavior)) {
                return;
            }
        ...
        private void addActionToExpand(
                ...
                @NonNull final T appBarLayout,
                @NonNull AccessibilityNodeInfoCompat.AccessibilityActionCompat action,
                ...
                if (curOffset != newOffset) {
                    ...
                    if (offsetChanged) {
                        ...
                        for (int i = 0; i < appBarLayout.getChildCount(); i++) {
                            MyAppbarLayout.LayoutParams params = (MyAppbarLayout.LayoutParams) appBarLayout.getChildAt(i).getLayoutParams();
                            MyAppbarLayout.ChildScrollEffect scrollEffect = params.getScrollEffect();
                            if (scrollEffect != null
                                    && (params.getScrollFlags() & MyAppbarLayout.LayoutParams.SCROLL_FLAG_SCROLL) != 0) {
        ...
        private void updateAppBarLayoutDrawableState(
            ...
            if (child != null) {
                ...
                if ((flags & MyAppbarLayout.LayoutParams.SCROLL_FLAG_SCROLL) != 0) {
                    ...
                    if (direction > 0
                            && (flags
                            & (MyAppbarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS
                            | MyAppbarLayout.LayoutParams.SCROLL_FLAG_ENTER_ALWAYS_COLLAPSED))
                            != 0) {
        ...
        private boolean shouldJumpElevationState(@NonNull CoordinatorLayout parent, @NonNull T layout) {
            ...
            for (int i = 0, size = dependencies.size(); i < size; i++) {
                ...
                if (behavior instanceof MyAppbarLayout.ScrollingViewBehavior) {
                    return ((MyAppbarLayout.ScrollingViewBehavior) behavior).getOverlayTop() != 0;
                }
            }
            return false;
        }
        ...
        private static View getAppBarChildOnOffset(
                @NonNull final MyAppbarLayout layout, final int offset) {
            final int absOffset = abs(offset);
            for (int i = 0, z = layout.getChildCount(); i < z; i++) {
                final View child = layout.getChildAt(i);
                if (absOffset >= child.getTop() && absOffset <= child.getBottom()) {
        ...
        public Parcelable onSaveInstanceState(@NonNull CoordinatorLayout parent, @NonNull T abl) {
            ...
            for (int i = 0, count = abl.getChildCount(); i < count; i++) {
                ...
                if (child.getTop() + offset <= 0 && visBottom >= 0) {
                    final MyAppbarLayout.BaseBehavior.SavedState ss = new MyAppbarLayout.BaseBehavior.SavedState(superState);
        ...
        public void onRestoreInstanceState(
            ...
            if (state instanceof MyAppbarLayout.BaseBehavior.SavedState) {
                savedState = (MyAppbarLayout.BaseBehavior.SavedState) state;
        ...
        /** A {@link Parcelable} implementation for {@link MyAppbarLayout}. */
        protected static class SavedState extends AbsSavedState {
            ...
            public static final Creator<MyAppbarLayout.BaseBehavior.SavedState> CREATOR =
                    new ClassLoaderCreator<MyAppbarLayout.BaseBehavior.SavedState>() {
                        @NonNull
                        @Override
                        public MyAppbarLayout.BaseBehavior.SavedState createFromParcel(@NonNull Parcel source, ClassLoader loader) {
                            return new MyAppbarLayout.BaseBehavior.SavedState(source, loader);
                        }

                        @Nullable
                        @Override
                        public MyAppbarLayout.BaseBehavior.SavedState createFromParcel(@NonNull Parcel source) {
                            return new MyAppbarLayout.BaseBehavior.SavedState(source, null);
                        }

                        @NonNull
                        @Override
                        public MyAppbarLayout.BaseBehavior.SavedState[] newArray(int size) {
                            return new MyAppbarLayout.BaseBehavior.SavedState[size];
                        }
                    };
        }
    }
     ...
     * scrolling to automatically scroll any {@link MyAppbarLayout} siblings.
     */
    public static class ScrollingViewBehavior extends HeaderScrollingViewBehavior {
        ...
        public void onDependentViewRemoved(
                @NonNull CoordinatorLayout parent, @NonNull View child, @NonNull View dependency) {
            if (dependency instanceof MyAppbarLayout) {
        ...
        public boolean onRequestChildRectangleOnScreen(
                ...
                boolean immediate) {
            final MyAppbarLayout header = findFirstDependency(parent.getDependencies(child));
            if (header != null) {
        ...
        private void offsetChildAsNeeded(@NonNull View child, @NonNull View dependency) {
            ...
            if (behavior instanceof MyAppbarLayout.BaseBehavior) {
                // Offset the child, pinning it to the bottom the header-dependency, maintaining
                // any vertical gap and overlap
                final MyAppbarLayout.BaseBehavior ablBehavior = (MyAppbarLayout.BaseBehavior) behavior;
        ...
        float getOverlapRatioForOffset(final View header) {
            if (header instanceof MyAppbarLayout) {
                final MyAppbarLayout abl = (MyAppbarLayout) header;
        ...
        private static int getAppBarLayoutOffset(@NonNull MyAppbarLayout abl) {
            final CoordinatorLayout.Behavior behavior =
                    ((CoordinatorLayout.LayoutParams) abl.getLayoutParams()).getBehavior();
            if (behavior instanceof MyAppbarLayout.BaseBehavior) {
                return ((MyAppbarLayout.BaseBehavior) behavior).getTopBottomOffsetForScrollingSibling();
            }
            return 0;
        }
        ...
        @Nullable
        @Override
        MyAppbarLayout findFirstDependency(@NonNull List<View> views) {
            for (int i = 0, z = views.size(); i < z; i++) {
                View view = views.get(i);
                if (view instanceof MyAppbarLayout) {
                    return (MyAppbarLayout) view;
                }
            }
            return null;
        }
        ...
        int getScrollRange(View v) {
            if (v instanceof MyAppbarLayout) {
                return ((MyAppbarLayout) v).getTotalScrollRange();
            } else {
        ...
        private void updateLiftedStateIfNeeded(View child, View dependency) {
            if (dependency instanceof MyAppbarLayout) {
                MyAppbarLayout appBarLayout = (MyAppbarLayout) dependency;
    ...
    public abstract static class ChildScrollEffect {
        ...
        public abstract void onOffsetChanged(
                @NonNull MyAppbarLayout appBarLayout, @NonNull View child, float offset);
    }
     ...
     */
    public static class CompressChildScrollEffect extends MyAppbarLayout.ChildScrollEffect {
        ...
        private static void updateRelativeRect(Rect rect, MyAppbarLayout appBarLayout, View child) {
